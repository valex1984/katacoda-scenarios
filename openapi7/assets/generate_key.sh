...
plan_id=$(curl -s -u admin:admin \
    $API_URL/management/organizations/DEFAULT/environments/DEFAULT/apis/$api_id \
    |jq -cr ' .plans[] | select( .name | contains("http-plan-access-all"))| .id')

#create application
app_id=$(curl -s -u user:password -X POST \
     -H "Content-Type:application/json;charset=UTF-8" \
     -d '{"name":"Demo Application","description":"Client for httpbin api"}' \
